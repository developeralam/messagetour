...
new #[Layout('components.layouts.admin')] #[Title('Booking List')] class extends Component {
    ...
    public function mount()
    {
        ...
        $this->types = [['id' => \App\Models\HotelRoomBooking::class, 'name' => 'Hotel Room Booking'], ['id' => \App\Models\TourBooking::class, 'name' => 'Tour Booking'], ['id' => \App\Models\TravelProductBooking::class, 'name' => 'Gear Booking'], ['id' => \App\Models\VisaBooking::class, 'name' => 'Visa Booking'], ['id' => \App\Models\CarBooking::class, 'name' => 'Car Booking']];
    ...
    {
        return [['key' => 'id', 'label' => '#'], ['key' => 'user.name', 'label' => 'Customer Name'], ['key' => 'phone', 'label' => 'Customer Mobile'], ['key' => 'booking_type', 'label' => 'Booking Type'], ['key' => 'booking_item', 'label' => 'Booking Item'], ['key' => 'booking_item_type', 'label' => 'Item Type'], ['key' => 'coupon', 'label' => 'Coupon'], ['key' => 'coupon_amount', 'label' => 'Discount'], ['key' => 'paymentgateway.name', 'label' => 'Payment Gateway'], ['key' => 'total_amount', 'label' => 'Total Amount'], ['key' => 'payment_status', 'label' => 'Payment Status'], ['key' => 'status', 'label' => 'Booking Status'], ['key' => 'action_by', 'label' => 'Last Action By']];
    }

    /**
     * Retrieve a paginated list of bookings with their associated users.
     *
     * This function allows searching by `business_name` and applies eager loading
     * to optimize query performance.
     *
     * @return \Livewire\WithPagination Paginated list of bookings.
     */
    public function bookings()
    {
        return Order::query()
            ->with(['user', 'coupon', 'paymentgateway'])
            ->where('sourceable_type', '!=', \App\Models\TravelProductBooking::class) // <-- exclude TravelProductBooking
...
<div>
    ...
    <x-card>
        <x-table :headers="$headers" :rows="$bookings" with-pagination>
            ...
            @scope('cell_booking_type', $order)
                @php
                    $bookingTypes = [
                        \App\Models\TourBooking::class => 'Tour',
                        \App\Models\HotelRoomBooking::class => 'Hotel Room',
                        \App\Models\CarBooking::class => 'Car Rental',
                        \App\Models\VisaBooking::class => 'Visa',
                    ];
            ...
            @scope('cell_booking_item_type', $order)
                @php
                    $booking = $order->sourceable;
                    $type = match (true) {
                        $booking instanceof App\Models\TourBooking => $booking->tour?->type->label(),
                        $booking instanceof App\Models\VisaBooking => $booking->visa?->type->label(),
                        $booking instanceof App\Models\CarBooking => $booking->car?->car_type->label(),
                        $booking instanceof App\Models\HotelRoomBooking => $booking->hotelbookingitems
                            ?->first()
                            ?->room?->type->label(),
                        default => null,
                    };
                @endphp
                <x-badge value="{{ $type ?? '-' }}" class="bg-primary text-white p-3 text-xs" />
            @endscope
            ...
            @scope('cell_coupon_amount', $order)
                @if ($order->coupon_amount > 0)
                    BDT {{ number_format($order->coupon_amount) }}
                @else
                    0
                @endif
            @endscope
            ...
            @scope('cell_total_amount', $order)
                BDT {{ number_format($order->total_amount) }}
            @endscope

            @scope('cell_coupon', $order)
