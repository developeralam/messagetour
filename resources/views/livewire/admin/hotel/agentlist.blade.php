...
new #[Layout('components.layouts.admin')] #[Title('Agent Hotel List')] class extends Component {
    use WithPagination;
    use Toast;
    ...
    public function approve(Hotel $agent)
    {
        try {
            $agent->update([
    ...
    {
        return [['key' => 'id', 'label' => '#'], ['key' => 'thumbnail', 'label' => 'Thumbnail'], ['key' => 'name', 'label' => 'Hotel Name'], ['key' => 'country.name', 'label' => 'Country'], ['key' => 'address', 'label' => 'Address'], ['key' => 'rooms_count', 'label' => 'Total Room'], ['key' => 'is_featured', 'label' => 'Is Featured'], ['key' => 'status', 'label' => 'Status'], ['key' => 'action_id', 'label' => 'Last Action By']];
    }
    public function hotels()
    {
        return Hotel::query()->withCount('rooms')->whereHas('createdBy', fn($q) => $q->where('type', UserType::Agent))->when($this->search, fn(Builder $q) => $q->where('name', 'LIKE', "%$this->search%"))->orderBy(...array_values($this->sortBy))->paginate(10);
    }
...
<div>
    <x-header title="Agent Hotel List" separator size="text-xl" class="bg-white px-2 pt-2">
    ...
    <x-card>
        <x-table :headers="$headers" :rows="$hotels" :sort-by="$sortBy" with-pagination>
            ...
            @scope('cell_thumbnail', $hotel)
                <x-avatar image="{{ $hotel->thumbnail_link ?? '/empty-product.png' }}" class="!w-10" />
            @endscope
