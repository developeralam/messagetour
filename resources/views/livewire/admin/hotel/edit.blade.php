...
new #[Layout('components.layouts.admin')] #[Title('Hotel Edit')] class extends Component {
    ...
    public function mount()
    {
        if ($this->hotel) {
            ...
            $this->checkin_time = Carbon::parse($this->hotel->checkin_time)->format('H:i');
            $this->checkout_time = Carbon::parse($this->hotel->checkout_time)->format('H:i');
    ...
    public function updateHotel()
    {
        ...
        try {
            ...
            $this->hotel->update([
                ...
                'checkin_time' => ($this->hotel->checkin_time = Carbon::createFromFormat('H:i', $this->checkin_time)->format('H:i:s')),
                'checkout_time' => ($this->hotel->checkout_time = Carbon::createFromFormat('H:i', $this->checkout_time)->format('H:i:s')),
            ...
            $this->success('Hotel Updated Successfully', redirectTo: '/admin/hotel/list');
        } catch (\Throwable $th) {
...
<div>
    ...
    @endpush
    <x-header title="Update {{ $hotel->name }} Hotel Details" separator size="text-xl" class="bg-white px-2 pt-2">
        <x-slot:actions>
            ...
            <div class="col-span-2">
                <x-card>
                    ...
                    <div class="grid grid-cols-3 gap-2">
                        <x-input label="Hotel Name" class="mb-4" wire:model="name" placeholder="Hotel Name"
                            required />
                        <x-input label="Hotel Address" class="mb-4" wire:model="address" placeholder="Ex:CoxsBazar"
                            required />
                ...
                <x-card class="mt-2">
                    ...
                    <div class="grid grid-cols-3 gap-2 mb-4">
                        <x-choices label="Country" :options="$countries" wire:model.live="country_id" single required
                            placeholder="Select One" />
                        <x-choices label="Division" :options="$divisions" wire:model.live="division_id" single required
                            placeholder="Select One" />
                        <x-choices label="District" :options="$districts" wire:model="district_id" single required
                            placeholder="Select One" />
                    </div>
                    <div wire:ignore class="mb-4">
                        <label for="description" class="font-normal text-sm">Hotel Description</label>
                        <textarea wire:model="description" id="description" cols="30" rows="10">{{ $hotel->description }}</textarea>
                    </div>
            ...
            <div class="col-span-1">
                ...
                <x-card class="mt-2">
                    ...
                    @php
                        $config = ['guides' => true];
                    @endphp
                    <x-file class="mb-4" label="Thumbnail" wire:model="thumbnail" crop-after-change :crop-config="$config">
                        <img src="{{ $this->hotel->thumbnail_link ?? asset('/empty-hotel.png') }}"
                    ...
                    <x-slot:actions>
                        ...
                        <x-button type="submit" label="Update Hotel" class="btn-primary btn-sm"
                            spinner="updateHotel" />
                    </x-slot:actions>
                </x-card>
            </div>
