...
new #[Layout('components.layouts.admin')] #[Title('Add New Room')] class extends Component {
    use WithFileUploads, Toast, WithMediaSync, InteractsWithImageUploads;
    ...
    {
        $response = Hotel::with('rooms')->find($hotel);
        $this->library = collect();
        if ($response) {
            $this->hotel = $response;
            $this->aminities();
        } else {
            $this->error('Hotel Not Found', redirectTo: '/admin/hotel/list');
        }
    ...
    public function storeRoom()
    {
        ...
        try {
            ...
            $this->success('Room Added Successfully', redirectTo: '/admin/hotel/' . $this->hotel->id . '/room/list');
        } catch (\Throwable $th) {
...
<div>
    <x-header title="Add New Room In - {{ $hotel->name }} Hotel" size="text-xl" separator class="bg-white px-2 pt-2">
        <x-slot:actions>
            <x-button label="Room List" icon="fas.arrow-left" link="/admin/hotel/{{ $hotel->id }}/room/list"
        ...
        <div class="grid grid-cols-3 gap-4 items-start" x-cloak>
            ...
            <div class="col-span-1">
                ...
                <x-card class="mt-4">
                    ...
                    <x-menu-separator />
                    <x-slot:actions>
                        <x-button label="Room List" link="/admin/hotel/{{ $hotel->id }}/room/list" class="btn-sm" />
