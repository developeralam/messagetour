...
new #[Layout('components.layouts.admin')] #[Title('Update Room')] class extends Component {
    ...
    public function updateRoom()
    {
        ...
        try {
            ...
            $this->success('Room Updated Successfully', redirectTo: '/admin/hotel/' . $this->room->hotel_id . '/room/list');
        } catch (\Throwable $th) {
...
<div>
    <x-header title="Update {{ $room->room_no }} - In {{ $room->hotel->name }} Hotel" size="text-xl" separator class="bg-white px-2 pt-2">
        <x-slot:actions>
            <x-button label="Room List" icon="fas.arrow-left" link="/admin/hotel/{{ $room->hotel_id }}/room/list" class="btn-primary btn-sm" />
        </x-slot:actions>
    </x-header>
        ...
        <div class="grid grid-cols-3 gap-4 items-start" x-cloak>
            ...
            <div class="col-span-1">
                ...
                <x-card class="mt-4">
                    ...
                    <x-menu-separator />
                    <x-slot:actions>
                        <x-button label="Room List" link="/admin/hotel/{{ $room->hotel->id }}/room/list" class="btn-sm" />
