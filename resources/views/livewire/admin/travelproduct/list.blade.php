...
new #[Layout('components.layouts.admin')] #[Title('Travel Product List')] class extends Component {
    ...
    {
        return TravelProduct::query()->with('actionBy')->whereNull('created_by')->when($this->search, fn(Builder $q) => $q->whereAny(['title', 'stock', 'regular_price', 'offer_price'], 'LIKE', "%$this->search%"))->orderBy(...array_values($this->sortBy))->paginate(10);
    }
...
<div>
    <x-header title="Travel Product List" size="text-xl" separator class="bg-white px-2 pt-2">
        ...
        <x-slot:actions>
            <x-button label="Add Travel Product" icon="o-plus" no-wire-navigate link="/admin/travel-product/create"
    ...
    <x-card>
        <x-table :headers="$headers" :rows="$products" :sort-by="$sortBy" with-pagination>
            ...
            @scope('cell_thumbnail', $product)
                <x-avatar image="{{ $product->thumbnail_link ?? '/empty-product.png' }}" class="!w-10" />
            @endscope
            ...
            @scope('actions', $product)
                <div class="flex items-center gap-1">
                    ...
                    <x-button icon="s-pencil-square" no-wire-navigate
                        link="/admin/travel-product/{{ $product['id'] }}/edit" class="btn-neutral btn-action " />
                </div>
            @endscope
        </x-table>
