...
new #[Layout('components.layouts.customer')] #[Title('Corporate Query Update')] class extends Component {
    use Toast;
    ...
    public function updateQuery(): void
    {
        ...
        try {
            ...
            $this->success('Query updated successfully.', redirectTo: '/my-corporate-query');
        } catch (\Throwable $th) {
...
<div>
    <x-header title="Update Corporate Query" size="text-xl" separator class="bg-white px-2 pt-2">
        <x-slot:actions>
            <x-button label="Back" link="/my-corporate-query" class="btn-sm btn-primary" icon="fas.arrow-left" />
        </x-slot:actions>
    </x-header>
