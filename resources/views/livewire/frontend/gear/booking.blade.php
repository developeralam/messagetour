...
new #[Layout('components.layouts.app')] #[Title('Gear Booking')] class extends Component {
    ...
    private function updatePaymentGateway()
    {
        ...
        $this->delivery_charge = $gateway?->charge ?? 0;
        $this->updateTotalAmount();
    }
    ...
    public function storeGearBooking()
    {
        ...
        try {
            ...
            $tourCreator = User::find($this->gear->created_by); // Find the gear creator
            $message = $this->gear->title . ' booked by ' . auth()->user()->name;

            // Notify all admins
            Notification::send($admins, new OrderNotification($order, $message));

            // Notify the gear creator only if they exist and are NOT an admin
...
<div class="bg-gray-100 px-5 xl:px-0">
    @section('booking')
        <div class="flex items-center space-x-2 justify-center text-white mt-10 font-extrabold text-xl">
            ...
            <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 text-green-700" fill="none" stroke="currentColor"
                            ...
                            <div class="flex flex-col items-center justify-center gap-2">
                                ...
                                <div class="flex items-center border border-green-500 rounded-md overflow-hidden">
                                    <button wire:click="decrementQty"
                                        class="bg-green-500 hover:bg-green-600 text-white px-2 py-1">
                            ...
                            <div class="text-center md:text-right">
                                @php
                                    ...
                                    $finalDiscountPercentage =
                                        $finalDiscount > 0 ? round(($finalDiscount / $originalSubtotal) * 100) : 0;
                                @endphp
                                ...
                                <div class="flex items-center justify-end gap-1">
                                    @if ($finalDiscount > 0)
                                        <img loading="lazy" class="w-5 h-5"
                                            src="{{ asset('images/discount-mono.svg') }}" alt="Discount Icon">
                                    @endif
                    ...
                    <x-card x-cloak class="py-6 h-full overflow-hidden shadow-md">
                        ...
                        <div class="px-4">
                            <x-textarea class="custome-input-field" wire:model="additional_information"
                                label="Additional Information" placeholder="Additional Information" rows="5" />
                        </div>
                    </x-card>
                </div>
                                ...
                                @foreach ($gateways as $gateway)
                                    <div class="form-check mb-2">
                                        ...
                                        <label class="form-check-label text-sm"
                                            for="gateway-{{ $gateway->id }}">{{ $gateway->name }}</label>
                                    </div>
                                @endforeach
                            </div>
