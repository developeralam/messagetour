<div>
    <div x-data="{ tab: 'login' }" class="flex items-center justify-center bg-gray-100">
        <x-modal wire:model="show" boxClass="max-w-md">
            <x-card>
                <img src="{{ asset('logo.png') }}" alt="Logo" class="w-8/12 mx-auto">
                <div x-show="tab === 'login'" x-cloak>
                    ...
                    <x-form wire:submit="login" class="flex flex-col gap-y-4">
                        <input type="email" placeholder="customer@gmail.com" class="border-b pb-1 outline-none"
                            wire:model="email" required>
                        <div x-data="{ show: false }" class="flex items-center gap-x-1 border-b pb-1">
                            <input :type="show ? 'text' : 'password'" placeholder="Enter Your Password"
                                class="outline-none flex-grow" wire:model="password" required>
                        ...
                        <a href="{{ route('forgetpassword') }}"
                            class="text-sm text-blue-500 hover:underline font-semibold">Forgot
                            password?</a>
                ...
                <div x-show="tab === 'register'" x-cloak>
                    ...
                    <x-form wire:submit="register" class="flex flex-col gap-y-4">
                        <input type="text" placeholder="Enter Your Name" wire:model="name"
                            class="border-b pb-1 outline-none" required>
                        <input type="email" placeholder="Enter Your Email Address" class="border-b pb-1 outline-none"
                            wire:model="email" required>
                        <div x-data="{ show: false }" class="flex items-center gap-x-1 border-b pb-1">
                            <input :type="show ? 'text' : 'password'" placeholder="Enter Your Password"
                                class="outline-none flex-grow" wire:model="password" required>
                        ...
                        <div x-data="{ show: false }" class="flex items-center gap-x-1 border-b pb-1">
                            <input :type="show ? 'text' : 'password'" placeholder="Confirm Password"
                                class="outline-none flex-grow" wire:model="confirmation_password" required>
                            <svg x-show="!show" @click="show = true" xmlns="http://www.w3.org/2000/svg" fill="none"
                                viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"
                                class="size-4 cursor-pointer">
                                <path stroke-linecap="round" stroke-linejoin="round"
                            ...
                            <svg x-show="show" @click="show = false" xmlns="http://www.w3.org/2000/svg"
                ...
                <p class="text-center text-sm text-gray-600 mt-6">
                    <span x-show="tab === 'login'">Don't have an account? <a href="#"
                            class="text-green-500 hover:text-green-600 hover:underline italic"
                            @click.prevent="tab = 'register'">Register Now</a></span>
                    <span x-show="tab === 'register'">Already have an account? <a href="#"
                            class="text-green-500 hover:text-green-600 hover:underline italic"
                            @click.prevent="tab = 'login'">Login Here</a></span>
                </p>
                ...
                <a href="{{ route('social.redirect', ['provider' => 'google', 'type' => 'customer']) }}"
                    ...
                    <svg data-v-67f23e14="" xmlns="http://www.w3.org/2000/svg" width="20" height="20"
                        viewBox="0 0 18 18">
