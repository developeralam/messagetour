...
new #[Layout('components.layouts.partner')] #[Title('Add Vehicle')] class extends Component {
    ...
    public function storeCar()
    {
        ...
        try {
            ...
            $storedImagePath = $this->image ? $this->optimizeAndStoreImage($this->image, 'public', 'car', null, null, 75) : null;
            ...
            Car::create([
                ...
                'with_driver' => $this->with_driver ?? RentalType::WithDriver,
                'status' => CarStatus::Pending,
                'created_by' => auth()->user()->id,
            ]);
            $this->success('Vehicle Added Successfully', redirectTo: '/partner/vehicle/list');
        } catch (\Throwable $th) {
...
<div>
    ...
    @endpush
    <x-header title="Add New Vehicle" size="text-xl" separator class="bg-white px-2 pt-2">
        <x-slot:actions>
            <x-button label="Back" link="/partner/vehicle/list" class="btn-sm btn-primary" icon="fas.arrow-left" />
        </x-slot:actions>
    </x-header>
        ...
        <div class="grid grid-cols-3 gap-2" x-cloak>
            ...
            <div class="col-span-1">
                <x-card>
                    ...
                    <x-file wire:model="image" label="Image" accept="images/png, images/jpeg,images/jpg" class="mb-2" required />
                    <x-checkbox label="Ac Facility" wire:model="ac_facility" class="w-4 h-4" />

                    <x-slot:actions>
                        <x-button label="Vehicle List" link="/partner/vehicle/list" class="btn-sm" />
                        <x-button type="submit" label="Add Vehicle" class="btn-primary btn-sm" spinner="storeCar" />
                    </x-slot:actions>
                </x-card>
            </div>
        </div>
