...
new #[Layout('components.layouts.partner')] #[Title('Vehicle List')] class extends Component {
    ...
    <x-card>
        <x-table :headers="$headers" :rows="$cars" :sort-by="$sortBy" with-pagination>
            ...
            @scope('cell_status', $car)
                @if ($car->status != \App\Enum\CarStatus::Pending)
                ...
                @else
                    <x-badge value="Pending" class="bg-yellow-100 text-yellow-700 p-3 text-xs font-semibold" />
                @endif
            @endscope
