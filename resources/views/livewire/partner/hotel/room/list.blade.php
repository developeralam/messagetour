...
new #[Layout('components.layouts.partner')] #[Title('Hotel Room List')] class extends Component {
    ...
    {
        return [['key' => 'id', 'label' => '#'], ['key' => 'thumbnail', 'label' => 'Thumbnail'], ['key' => 'name', 'label' => 'Room Name'], ['key' => 'room_no', 'label' => 'Room No'], ['key' => 'type', 'label' => 'Room Type'], ['key' => 'regular_price', 'label' => 'Regular Price'], ['key' => 'offer_price', 'label' => 'Offer Price'], ['key' => 'status', 'label' => 'Status']];
    }
...
<div>
    <x-header title="Room List Of - {{ $hotel->name }}" separator size="text-xl" class="bg-white px-2 pt-2">
        <x-slot:middle class="!justify-end">
            <x-input icon="o-bolt" wire:model.live="search" placeholder="Search..." class="max-w-32" />
        </x-slot:middle>
        <x-slot:actions>
            <x-button label="Add Room" icon="o-plus" link="/partner/hotel/{{ $hotel->id }}/room/create"
            ...
            @scope('actions', $room)
                <div class="flex items-center gap-1">
                    ...
                    <x-button icon="s-pencil-square"
                        link="/partner/hotel/{{ $room['hotel_id'] }}/room/{{ $room['id'] }}/edit"
