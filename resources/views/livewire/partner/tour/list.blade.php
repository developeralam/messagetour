...
new #[Layout('components.layouts.partner')] #[Title('Tour List')] class extends Component {
    use WithPagination;
    use Toast;
...
<div>
    <x-header title="Tour List" separator size="text-xl" class="bg-white px-2 pt-2">
        <x-slot:middle class="!justify-end">
            <x-input icon="o-bolt" wire:model.live="search" placeholder="Search..." />
        </x-slot:middle>
        <x-slot:actions>
            <x-button label="Add Tour" icon="o-plus" no-wire-navigate link="/partner/tour/create"
                class="btn-primary btn-sm" />
        </x-slot:actions>
    </x-header>
    <x-card>
        <x-table :headers="$headers" :rows="$tours" with-pagination>
            ...
            @scope('cell_thumbnail', $tour)
                <x-avatar image="{{ $tour->thumbnail_link ?? '/empty-user.jpg' }}" class="!w-10" />
            @endscope
            ...
            @scope('actions', $tour)
                <div class="flex items-center gap-1">
                    ...
                    <x-button icon="s-pencil-square" no-wire-navigate link="/partner/tour/{{ $tour['id'] }}/edit"
