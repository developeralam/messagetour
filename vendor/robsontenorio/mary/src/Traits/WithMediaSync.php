...
trait WithMediaSync
{
    ...
    public function syncMedia(
        ...
        foreach ($this->{$files} as $index => $file) {
            ...
            // Original name from your getFileName() method
            $originalName = $this->getFileName($media);

            // Extract the base name without extension, then enforce .webp
            $baseName = pathinfo($originalName, PATHINFO_FILENAME);
            $webpName = $baseName.'.webp'; // Force .webp extension
            ...
            $filePath = $storage_subpath.'/'.$webpName;

            // $encoded is typically a string of the WebP bytes, so we can put it directly.
            Storage::disk($disk)->put($filePath, $encoded, $visibility);
            $url = Storage::disk($disk)->url($filePath);

            // $file = Storage::disk($disk)->putFileAs($storage_subpath, $encoded, $name, $visibility);
            // $url = Storage::disk($disk)->url($file);

            // Update library
            $media['url'] = $url.'?updated_at='.time();
            $media['path'] = str($storage_subpath)->finish('/')->append($webpName)->toString();
