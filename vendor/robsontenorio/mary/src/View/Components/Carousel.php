        ...
        return <<<'HTML'
            <div x-data="{
                ...
                @if(! $withoutIndicators)
                    <div class="absolute rounded-xl bottom-3 md:bottom-5 left-1/2 z-[2] flex -translate-x-1/2 gap-4 md:gap-3 bg-white/75 px-1.5 py-1 md:px-2" role="group" aria-label="slides" >
                        <template x-for="(slide, index) in slides">
                            <button class="size-2.5 cursor-pointer rounded-full transition hover:scale-125 bg-slate-700" @click="currentSlideIndex = index + 1" :class="[currentSlideIndex === index + 1 ? 'bg-slate-700' : 'bg-slate-700/50']"></button>
                        </template>
                    </div>
                @endif
            </div>
