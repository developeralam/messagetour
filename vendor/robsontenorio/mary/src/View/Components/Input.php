    ...
    public function render(): View|Closure|string
    {
        return <<<'BLADE'
            <div>
                ...
                {{-- STANDARD LABEL --}}
                @if($label && !$inline)
                    <label for="{{ $uuid }}" class="label label-text font-normal p-0">
                        <span>
                            {{ $label }}
                ...
                <div class="flex-1 relative">
                    ...
                    <input
                        ...
                        {{
                            $attributes
                                ...
                                ->class([
                                    'text-sm input input-primary w-full peer',
                                    'ps-10' => ($icon),
                                    'h-14' => ($inline),
                                    'pt-0' => ($inline && $label),
                    ...
                    {{-- INLINE LABEL --}}
                    @if($label && $inline)
                        <label for="{{ $uuid }}" class="text-sm absolute text-gray-400 duration-300 transform -translate-y-1 scale-75 top-[0.1rem] origin-left rtl:origin-right rounded px-2 peer-focus:px-2 peer-focus:text-blue-600 peer-focus:dark:text-blue-500 peer-placeholder-shown:scale-100 peer-placeholder-shown:-translate-y-1/2 peer-placeholder-shown:top-1/2 peer-focus:top-2 peer-focus:scale-75 peer-focus:-translate-y-1 @if($inline && $icon) start-9 @else start-3 @endif">
                            {{ $label }}
                        </label>
                    @endif
