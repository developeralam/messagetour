...
class MenuSub extends Component
{
    ...
    public function render(): View|Closure|string
    {
        ...
        return <<<'HTML'
                @aware(['activeBgColor' => 'bg-sidebar-active'])
                ...
                >
                    ...
                    <details :open="show" @if($submenuActive) open @endif @click.stop>
                        <summary @click.prevent="toggle()" @class(["hover:text-white px-2 my-[5px] hover:bg-default text-inherit", $activeBgColor => $submenuActive])>
                            @if($icon)
                                <x-mary-icon :name="$icon" class="inline-flex"  />
                            @endif
                            <span class="mary-hideable text-xs">{{ $title }}</span>
                        </summary>
